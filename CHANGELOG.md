...
- **`DrupalRector\Set\DrupalSetProvider`** — superseded by `DrupalSetList::COMPOSER_BASED`, but kept as a backward-compatibility layer so existing `withSetProviders(DrupalSetProvider::class)` configs keep working. Rector deprecated `SetProviderInterface` and `ComposerTriggeredSet`, which the provider is built on, so it will be removed in a later release.
