        ...
        if ($this->getName($node->name) === $this->deprecatedMethodName) {
            ...
            if (count($node->args) >= $this->optionsArgumentPosition) {
                ...
                if ($options->value->getType() === 'Expr_Variable') {
                    ...
                    $this->addComment($node, '// Rector notice: If your `options` argument contains a `target` key, you will need to use `\Drupal\core\Database\Database::getConnection(\'my_database\'). Drupal Rector could not yet evaluate the `options` argument since it was a variable.');
                }
            }
            else {
                $this->addComment($node, '// Rector notice: You will need to use `\Drupal\core\Database\Database::getConnection()` if you do not yet have access to the container here.');
            }
